...
function get_timespan_most_viewed($mode = '', $limit = 10, $days = 7, $display = true) {
	...
	if($most_viewed) {
		...
		foreach ($most_viewed as $post) {
			...
			$temp .= "<span class=\"list-date\">$post_views</span>".__('', 'wp-postviews')."<li><span class='li-icon li-icon-$i'>$i.</span><a href=\"".get_permalink()."\" title=\"".get_the_title()."\">$post_title</a></li>";
			$i++;
		}
	} else {
