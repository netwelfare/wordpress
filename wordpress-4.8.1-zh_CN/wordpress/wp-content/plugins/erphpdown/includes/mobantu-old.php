...
function erphp_register_extra_fields($user_id, $password="", $meta=array()) {
	global $wpdb;
	if(isset($_COOKIE["erphprefid"])){
		$sql = "update $wpdb->users set father_id='".esc_sql($_COOKIE["erphprefid"])."' where ID=".$user_id;
		$wpdb->query($sql);
	}
}
...
function erphpdown_install() {
	...
	$up5to6="ALTER TABLE '".$wpdb->users."' ADD  'father_id' INT( 10 ) NOT NULL DEFAULT  '0'";
	$wpdb->query($up5to6);
	
	$up8to9="ALTER TABLE '".$wpdb->icemoney."' modify column ice_num varchar(50)";
	$wpdb->query($up8to9);

	add_option( 'erphpdown_version', $erphpdown_version );
}
...
function checkDownLog($uid,$ip){
	global $wpdb;
	$userType=getUsreMemberType();
	if($userType) return true;
	else{//1小时内只能下载一个免费文件
		$result = $wpdb->get_var("select ice_id from $wpdb->down where ice_user_id=".$uid." and ice_time >= date_add(now(),interval -1 hour)");
...
function erphp_modify_dashboard_widgets() {
	...
	add_meta_box( 'erphpdown_dashboard_widget', '我的下载/查看', 'erphpdown_dashboard_widget_function','dashboard', 'normal', 'core' );
}
function erphpdown_dashboard_widget_function() {
	...
	echo '下载：'.$total_trade.'&nbsp;&nbsp;消费：'.sprintf("%.2f",$userMoney->ice_get_money).get_option('ice_name_alipay').'&nbsp;&nbsp;剩余：'.sprintf("%.2f",$okMoney).get_option('ice_name_alipay').'<br />';
	echo '<ul>';
