...
if($_POST)
{
	...
	if($_POST['paytype']){
		...
		{
			$url=get_bloginfo('url')."/wp-content/plugins/erphpdown/chinabank.php?ice_money=".$wpdb->escape($_POST['ice_money']);
		}
		elseif(isset($_POST['paytype']) && $paytype==1)
		{
			$url=get_bloginfo('url')."/wp-content/plugins/erphpdown/alipay.php?ice_money=".$wpdb->escape($_POST['ice_money']);
		}
		elseif(isset($_POST['paytype']) && $paytype==7)
		{
			$url=get_bloginfo('url')."/wp-content/plugins/erphpdown/tenpay.php?ice_money=".$wpdb->escape($_POST['ice_money']);
		}
		elseif(isset($_POST['paytype']) && $paytype==5)
		{
			$url=get_bloginfo('url')."/wp-content/plugins/erphpdown/baolong/index.php?ice_money=".$wpdb->escape($_POST['ice_money']);
		}
		elseif(isset($_POST['paytype']) && $paytype==4)
		{
			$url=get_bloginfo('url')."/wp-content/plugins/erphpdown/weixin/example/weixin.php?ice_money=".$wpdb->escape($_POST['ice_money']);
		}
		elseif(isset($_POST['paytype']) && $paytype==8)
		{
			$url=get_bloginfo('url')."/wp-content/plugins/erphpdown/alipay_jk.php?ice_money=".$wpdb->escape($_POST['ice_money']);
		}
		elseif(isset($_POST['paytype']) && $paytype==2)
		{
			$url=get_bloginfo('url')."/wp-content/plugins/erphpdown/paypal.php?ice_money=".$wpdb->escape($_POST['ice_money']);
		}
		else{
...
<section class="container">
	<link rel='stylesheet' href="<?php echo get_template_directory_uri().'/css/erphpdown.css' ?>" type='text/css' media='all' />	
		...
		<div class="pagecontent profile-content">
			...
			<div id="infocenter">
				...
				<div class="profile-form">
					...
					<dl class="dl-horizontal">
						<dt>昵称</dt>
						<dd>
							<input type="text" class="profile-input" style="height:30px;" id="mm_name" name="mm_name" value="<?php echo esc_attr( $profileuser->nickname ) ?>">
						</dd>
					</dl>
					<dl class="dl-horizontal">
						<dt>邮箱</dt>
					...
					<dl class="dl-horizontal">
						<dt>网址</dt>
						<dd>
							<input type="text" class="profile-input" style="height:30px;" id="mm_url" name="mm_url" value="<?php echo esc_attr( $profileuser->user_url ) ?>">
						</dd>
					</dl>
					<dl class="dl-horizontal">
						<dt>简介</dt>
					...
					$("#doprofile").click(function(){ 
						...
						else
						{
							...
							$.ajax({
								type: "post",
								//async: false,
								url: "<?php echo get_template_directory_uri() ?>/ajax/ajax-profile.php",
			...
			?>
				<div class="profile-form">
					...
					<dl class="dl-horizontal">
						<dt>支付宝账号</dt>
						<dd><?php if(!$userAli){?>
                        <input type="text" id="ice_alipay" style="height:30px" name="ice_alipay" maxlength="50" size="50" />
                    <?php }else{
					...
					<dl class="dl-horizontal">
						<dt>支付宝姓名</dt>
						<dd><?php if(!$userAli){?>
                        <input type="text" id="ice_name" style="height:30px" name="ice_name" maxlength="50" size="50" />
                    <?php }else{
					...
					<dl class="dl-horizontal">
						<dt>提现金额</dt>
						<dd>
							<input type="text" id="ice_money" style="height:30px" name="ice_money" maxlength="50" size="50" />（余额：<?php echo sprintf("%.2f",$okMoney)?><?php echo get_option(ice_name_alipay)?>）
						</dd>
					</dl>
					<dl class="dl-horizontal">
						<dt></dt>
			...
			<div id="changepass">
				...
				<div class="profile-form">
                    ...
                    <dl class="dl-horizontal" style="display:none">
						<dt>用户名</dt>
						<dd>
							<input type="text" id="mm_username" style="height:30px;" name="log" value="<?php echo esc_attr( $profileuser->user_login ); ?>">
						</dd>
					</dl>
                    <dl class="dl-horizontal" style="display:none">
						<dt>旧密码</dt>
						<dd>
							<input type="password" style="height:30px;" id="mm_pass_old" name="pwd" value="">
						</dd>
					</dl>
					<dl class="dl-horizontal">
						<dt>新密码</dt>
						<dd>
							<input type="password" style="height:30px;" id="mm_pass_new" name="mm_pass_new" value="">
						</dd>
					</dl>
					<dl class="dl-horizontal">
						<dt>重复密码</dt>
						<dd>
							<input type="password" style="height:30px;" id="mm_pass_new2" name="mm_pass_new2" value="">
						</dd>
					</dl>
					<dl class="dl-horizontal">
						<dt></dt>
			...
			<?php }elseif($_GET["pd"]=='money'){ ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////个人资产
				...
				<div style="width:100%;">
                    ...
                    消费<?php echo get_option('ice_name_alipay');?>：<?php echo intval($userMoney->ice_get_money)?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;剩余<?php echo get_option('ice_name_alipay');?>：<?php echo $okMoney?><br />
                ...
                <form action="" method="post" target="blank" onSubmit="return checkFm();">
                        ...
                        <table class="form-table">
                            <tr>
                				<td>
                				<input type="text" id="ice_money" style="height:30px;" name="ice_money" maxlength="50" size="50" required="required"/>（请输入一个整数金额，1人民币 = 1<?php echo get_option('ice_name_alipay')?>）
                				</td>
                            </tr>
                            <tr>
                                <td>
                                ...
                                <?php }?>
                                <?php if(get_option('erphpdown_baolong_uid')){?> 
                                <input type="radio" id="paytype5" class="paytype" checked name="paytype" value="5" onclick="checkCard()"/>宝龙支付(￥人民币)&nbsp;    
                                <?php }?> 
                ...
                <h1 style="border-bottom:1px solid #999999;width:100%;color:#F04243;">升级VIP</h1>
                	<table class="form-table">
                        ...
                        <tr>
                            <td>
                                <input type="radio" id="userType" name="userType" value="10" checked />终身会员（<?php echo $ciphp_life_price.get_option('ice_name_alipay')?> ）&nbsp;<input type="radio" id="userType" name="userType" value="9" />包年会员（<?php echo $ciphp_year_price.get_option('ice_name_alipay')?> ）&nbsp;<input type="radio" id="userType" name="userType" value="8" />包季会员（<?php echo $ciphp_quarter_price.get_option('ice_name_alipay')?> ）&nbsp;<input type="radio" id="userType" name="userType" value="7" />包月会员（<?php echo $ciphp_month_price.get_option('ice_name_alipay')?> ）
                            </td>
                        </tr>
                        <tr>
